...
@@&&fc_def_empty_var. gcp360_lin_prepare_wh

col gcp360_where_skip new_v gcp360_where_skip NOPRI
select nvl2(q'[&&gcp360_lin_prepare_wh.]','','--') gcp360_where_skip from dual;
col gcp360_where_skip clear

WITH t1 AS (
  SELECT SUM("cost/myCost") COMPUTEDAMOUNT,
         &&gcp360_lin_prepare_id. ITEM_ID,
         &&gcp360_lin_prepare_ds. ITEM_DESC
  FROM   GCP360_REPORTS_COST
&&gcp360_where_skip.  WHERE  &&gcp360_lin_prepare_wh.
  GROUP BY &&gcp360_lin_prepare_id.
...
WITH t1 AS (
  SELECT SUM("cost/myCost") COMPUTEDAMOUNT,
         TO_TIMESTAMP("lineItem/intervalUsageEnd",'&&gcp360_usage_tzcolformat.') ENDTIMEUTC,
         &&gcp360_lin_prepare_id. ITEM_ID
  FROM   GCP360_REPORTS_COST
&&gcp360_where_skip.  WHERE  &&gcp360_lin_prepare_wh.
  GROUP BY TO_TIMESTAMP("lineItem/intervalUsageEnd",'&&gcp360_usage_tzcolformat.'), &&gcp360_lin_prepare_id.
),
trange as (
  select trunc(min(TO_TIMESTAMP("lineItem/intervalUsageEnd",'&&gcp360_usage_tzcolformat.')),'HH24') min,
         trunc(max(TO_TIMESTAMP("lineItem/intervalUsageEnd",'&&gcp360_usage_tzcolformat.')),'HH24') max
  FROM   GCP360_REPORTS_COST
),
...
UNDEF gcp360_lin_prepare_id gcp360_lin_prepare_ds gcp360_lin_prepare_wh gcp360_where_skip