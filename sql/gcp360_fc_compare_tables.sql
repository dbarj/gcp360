DEF gcp360_in_table_before = "&&1."
DEF gcp360_in_table_after  = "&&2."
DEF gcp360_in_table_file   = "&&3."

UNDEF 1 2 3

@@&&fc_spool_start.
SET SERVEROUT ON
SPO &&gcp360_in_table_file.;

DECLARE

  V_TAB_B4 VARCHAR2(30) := '&&gcp360_in_table_before.';
  V_TAB_AF VARCHAR2(30) := '&&gcp360_in_table_after.';

  CURSOR L_ADD IS (SELECT DISTINCT ID,DISPLAY_NAME FROM &&gcp360_in_table_after. T1 WHERE NOT EXISTS (SELECT 1 FROM &&gcp360_in_table_before. T2 WHERE T1.ID=T2.ID));
  CURSOR L_REM IS (SELECT DISTINCT ID,DISPLAY_NAME FROM &&gcp360_in_table_before. T2 WHERE NOT EXISTS (SELECT 1 FROM &&gcp360_in_table_after. T1 WHERE T1.ID=T2.ID));
  CURSOR L_COLS IS (SELECT T1.COLUMN_NAME COL1, T2.COLUMN_NAME COL2
                    ...
                    AND    T1.OWNER = '&&gcp360_user_curschema.'
...
UNDEF gcp360_in_table_before gcp360_in_table_after gcp360_in_table_file
---