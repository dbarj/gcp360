-- webvowl configuration for GCP360 - Rodrigo Jorge

DEF gcp360_webvowl_zip   = '&&moat369_sw_base./&&moat369_sw_misc_fdr./webvowl_1.1.7.zip'

DEF gcp360_webvowl_fdr  = 'webvowl'
DEF gcp360_webvowl_data = 'data'

@@&&fc_def_output_file. gcp360_webvowl_json      'gcp360.json'
@@&&fc_def_output_file. gcp360_webvowl_qry       'webvowl_qry.sql'
@@&&fc_def_output_file. gcp360_webvowl_fdr_path  '&&gcp360_webvowl_fdr.'
@@&&fc_def_output_file. gcp360_webvowl_data_path '&&gcp360_webvowl_data.'

HOS mkdir &&gcp360_webvowl_fdr_path.
HOS unzip -d &&gcp360_webvowl_fdr_path. &&gcp360_webvowl_zip. >> &&moat369_log3.

HOS &&cmd_awk. '{sub(/foaf/,"gcp360")}1' &&gcp360_webvowl_fdr_path./js/webvowl.app.js > &&gcp360_webvowl_fdr_path./js/webvowl.app.js.new
HOS mv &&gcp360_webvowl_fdr_path./js/webvowl.app.js.new &&gcp360_webvowl_fdr_path./js/webvowl.app.js

HOS cd &&gcp360_webvowl_fdr_path./../; zip -rm &&moat369_zip_filename. &&gcp360_webvowl_fdr./css/        >> &&moat369_log3.
HOS cd &&gcp360_webvowl_fdr_path./../; zip -rm &&moat369_zip_filename. &&gcp360_webvowl_fdr./js/         >> &&moat369_log3.
HOS cd &&gcp360_webvowl_fdr_path./../; zip -m  &&moat369_zip_filename. &&gcp360_webvowl_fdr./favicon.ico >> &&moat369_log3.
HOS cd &&gcp360_webvowl_fdr_path./../; zip -m  &&moat369_zip_filename. &&gcp360_webvowl_fdr./license.txt >> &&moat369_log3.

HOS rm -rf &&gcp360_webvowl_fdr_path.
...
FROM  (select id from GCP360_INSTANCES union
       select id from GCP360_DB_NODES union
       select distinct id from GCP360_SUBNETS union
       select distinct 'z_link_' || id from GCP360_SUBNETS union
       select id from GCP360_VCNS union
       select id from GCP360_PRIVATEIPS union
       select id from GCP360_REMOTE_PEERING union
       select id from GCP360_LOCAL_PEERING union
       select id from GCP360_PUBLICIPS union
       select 'z_link_' || id from GCP360_PUBLICIPS)),
t_insts_subs AS (
-- Compute Instances
SELECT t4.id,
       ...
       t2.subnet_id,
       t2.instance_id
FROM  GCP360_INSTANCES t1, GCP360_VNIC_ATTACHS t2, GCP360_VNICS t3, GCP360_PRIVATEIPS t4
...
SELECT t4.id,
       ...
       t2.subnet_id,
       t1.id
FROM  GCP360_DB_NODES t1, GCP360_VNICS t2, GCP360_SUBNETS t3, GCP360_PRIVATEIPS t4
...
SELECT t3.id,
       ...
       t6.id vcn_2_id
FROM   GCP360_VCNS t1,
       GCP360_DRG_ATTACHS t2,
       GCP360_REMOTE_PEERING t3,
       GCP360_REMOTE_PEERING t4,
       GCP360_DRG_ATTACHS t5,
       GCP360_VCNS t6
...
FROM   GCP360_VCNS t1,
       GCP360_LOCAL_PEERING t2,
       GCP360_VCNS t3,
       GCP360_LOCAL_PEERING t4
WHERE  t1.id = t2.vcn_id
AND    t1.lifecycle_state='AVAILABLE'
...
SELECT 'z_link_' || t4.id id,
       ...
       t2.instance_id,
       t4.id pubip_id
FROM   GCP360_INSTANCES t1,
       GCP360_VNIC_ATTACHS t2,
       GCP360_VNICS t3,
       GCP360_PUBLICIPS t4
WHERE  t1.id = t2.instance_id
AND    t2.lifecycle_state = 'ATTACHED'
...
FROM  GCP360_PUBLICIPS t1
WHERE exists (select 1 from t_insts_pubips where t1.id=t_insts_pubips.pubip_id)) ]';
END;
/

@@&&fc_spool_start.
SPO &&gcp360_webvowl_json.
PRO {
PRO   "_comment" : "Created with GCP360",
PRO   "header" : {
...
PRO     ],
PRO     "title" : {
PRO       "undefined" : "GCP360 Infrastructure View"
PRO     },
...
@@&&fc_spool_start.
SPO &&gcp360_webvowl_qry.
SELECT :sql_text FROM DUAL;
SPO OFF

GET &&gcp360_webvowl_qry.

SPO &&gcp360_webvowl_json. APP
/
PRO   ],
PRO   "classAttribute" : [
SPO OFF
...
@@&&fc_zip_driver_files. &&gcp360_webvowl_qry.
...
HOS mkdir &&gcp360_webvowl_data_path.
HOS mv &&gcp360_webvowl_json. &&gcp360_webvowl_data_path.

HOS cd &&gcp360_webvowl_data_path./../; zip -rm &&moat369_zip_filename. &&gcp360_webvowl_data. >> &&moat369_log3.
