...
-- BEGIN
--   FHANDLE := UTL_FILE.FOPEN('&&gcp360_obj_dir.', '&&gcp360_temp_extout.', 'w');
--   UTL_FILE.PUT(FHANDLE, '&&gcp360_in_source_file.');
...
-- 
-- -- Drop External Table
-- BEGIN EXECUTE IMMEDIATE 'DROP TABLE &&gcp360_user_curschema.."&&gcp360_temp_exttab." PURGE'; EXCEPTION WHEN OTHERS THEN NULL; END;
-- /
-- 
-- -- Create External Table with CLOB contents
-- CREATE TABLE &&gcp360_user_curschema.."&&gcp360_temp_exttab."
-- (
--   json_document CLOB
-- )
...
--           )
--           COLUMN TRANSFORMS (json_document FROM LOBFILE (json_filename) FROM ("&&gcp360_obj_dir.") CLOB)
--     )
--    LOCATION ('&&gcp360_temp_extout.')
-- )
-- REJECT LIMIT 0
-- NOPARALLEL
-- NOMONITORING
