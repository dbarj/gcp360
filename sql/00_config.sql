...
-- Defines GCP360 converter mode. Options are (ON_DEMAND, PRE_LOAD or OFF)
-- PRE_LOAD  = Convert all jsons inside tables in the beggining. (Recommended and default if omitted)
-- ON_DEMAND = Convert tables from jsons on-demand, as they are used in each section. Good for running specific sections only. eg: @gcp360 1a
-- OFF       = Don't convert json into tables. Tables must exist from a prior execution (combine this with gcp360_clean_on_exit = OFF).
-- DEF gcp360_load_mode = 'PRE_LOAD';

-- Defines GCP360 clean mode. Options are (ON or OFF)
-- ON  = Remove all loaded tables in the end. (Recommended and default if omitted)
-- OFF = Keep all loaded tables in the end. Good if you plan to run the gcp360 again later or check contents. In next execution, run with gcp360_load_mode = OFF.
