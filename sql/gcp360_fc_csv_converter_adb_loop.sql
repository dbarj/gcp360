...
SET TERM ON
PRO Loading "&&gcp360_in_source_file." in "&&gcp360_in_target_table.".
@@&&fc_set_term_off.

-- Start SPOOL to log file
@@&&fc_spool_start.
SET ECHO OFF FEED ON VER ON HEAD ON SERVEROUT ON
SPO &&gcp360_log_csv. APP;
...
BEGIN  
   DBMS_CLOUD.CREATE_EXTERNAL_TABLE(   
      table_name =>'&&gcp360_temp_exttab.',   
      credential_name =>'&&gcp360_adb_cred.',   
      file_uri_list =>'&&gcp360_adb_uri.&&gcp360_in_source_file.',
...
BEGIN EXECUTE IMMEDIATE 'DROP TABLE "&&gcp360_temp_exttab._NO_HEADER" PURGE'; EXCEPTION WHEN OTHERS THEN NULL; END;
/

BEGIN  
   DBMS_CLOUD.CREATE_EXTERNAL_TABLE(   
      table_name =>'&&gcp360_temp_exttab._NO_HEADER',   
      credential_name =>'&&gcp360_adb_cred.',   
      file_uri_list =>'&&gcp360_adb_uri.&&gcp360_in_source_file.',
...
DECLARE
   ...
   V_SEL_SQL CLOB;
BEGIN
  for i in (select column_name from all_tab_columns where owner=SYS_CONTEXT('userenv','current_schema') and table_name='&&gcp360_temp_exttab.' order by column_id)
  loop
    V_QRY := 'select ' || i.column_name || ' from "&&gcp360_temp_exttab." where rownum=1';
    execute immediate V_QRY into V_CSV_COL;
    BEGIN
      select TAB_COL_NAME into V_TAB_COL from "&&gcp360_temp_colcontrol." where CSV_COL_NAME=V_CSV_COL;
    ...
    if V_CSV_COL is not null then
      if V_TAB_COL is null then
        ...
        V_TAB_COL := V_CSV_COL;
        INSERT INTO "&&gcp360_temp_colcontrol." (CSV_COL_NAME,TAB_COL_NAME) values (V_CSV_COL,V_TAB_COL);
        COMMIT;
        DBMS_OUTPUT.PUT_LINE('Adding column ' || dbms_assert.enquote_name(V_CSV_COL,FALSE) || ' on &&gcp360_in_target_table..');
        EXECUTE IMMEDIATE 'alter table &&gcp360_in_target_table. add ' || dbms_assert.enquote_name(V_CSV_COL,FALSE) || ' varchar2(4000)';
      end if;
  ...
  V_QRY := 'insert /*+ append */ into &&gcp360_in_target_table. (' || V_INS_SQL || ') select ' || V_SEL_SQL || ' from "&&gcp360_temp_exttab._NO_HEADER"';
...
DROP TABLE "&&gcp360_temp_exttab._NO_HEADER" PURGE;
