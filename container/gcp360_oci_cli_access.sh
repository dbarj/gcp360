...
# Create oci-cli instance_principal access for GCP360
# Only use this script if the tenancy you are running GCP360 is the same you want to get the info.
# Run this script from Cloud Shell with Administrator account.

set -eo pipefail

echo -n "GCP360 Instance OCID: "
read v_instance_ocid
[ -z "${v_instance_ocid}" ] && echo "Instance OCID can't be null." && exit 1

v_dyngroup_def_name='GCP360_DG'
echo -n "Dynamic Group Name for GCP360 [${v_dyngroup_def_name}]: "
read v_dyngroup_name
[ -z "${v_dyngroup_name}" ] && v_dyngroup_name="${v_dyngroup_def_name}"

v_policy_def_name='GCP360_Policy'
echo -n "Policy Name for GCP360 [${v_policy_def_name}]: "
read v_policy_name
...
--description 'Group to handle oci-cli calls from the host of GCP360.' 2>&1 >/dev/null) || true
...
--description 'Policy to handle oci-cli calls from the host of GCP360.' 2>&1 >/dev/null) || true
